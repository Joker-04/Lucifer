...
PICS = (environ.get('PICS', 'https://envs.sh/o1b.jpg https://envs.sh/o1w.jpg')).split() #SAMPLE PIC
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002030633559 -1002086541598').split()] #Channel id for auto indexing ( make sure bot is admin )
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002417525913')) #Log channel id ( make sure bot is admin )
DEENDAYAL_MOVIE_UPDATE_CHANNEL = int(environ.get('DEENDAYAL_MOVIE_UPDATE_CHANNEL', '-1002977300234')) #Notification of those who verify will be sent to your channel. Enter the ID of the channel you want to send notification to here.
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002417525913')) # add premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '-1002070573213 -1003007306706') #Channel / Group Id for force sub ( make sure bot is admin )
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1002031166889').split()]
...
# Verify
VERIFY = bool(environ.get('VERIFY', Fals)) # Verification On ( True ) / Off ( False )
...
#Channel & Group link 
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/+OD_dO1r6BAw3Yzhl')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/FilmyEraOfficial')
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/Who_I_AM02')
